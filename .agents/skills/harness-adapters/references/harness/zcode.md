...
No supervised zcode worker has run, so this reference is the Phase A design, not observed behavior, and zcode is not dispatchable until the live verification gate below passes.
...
| Busy state | No semantic source: the Phase A design adds a zcode arm to `../../../../../bin/fm-busy-lib.sh` in the grok/rovo/agy rendered-tail screen-scrape shape, and its busy signature is unobserved until the live gate captures a real zcode turn. |
...
## Crewmate and scout only, pending verification

zcode is scoped to crewmate and scout launches only; the live verification gate below owns when that scope becomes dispatchable.
A secondmate is a firstmate instance and needs a primary supervision protocol; zcode has none, so the Phase A spawn arm must refuse a zcode secondmate, the muse/gemini/agy shape in `../../../../../bin/fm-spawn.sh`.
Until the adapter code lands, `../../../../../bin/fm-spawn.sh` refuses every zcode spawn as an unknown harness.

## Supervision recipe, Phase A design

This recipe does not become operative until the live verification gate below passes, and it lives here rather than in `docs/supervision-protocols/` because that directory is the rendered primary-wake-protocol set whose emptiness for crewmate/scout-only harnesses is load-bearing in `../../../../../bin/fm-spawn.sh`'s refusal comments.
