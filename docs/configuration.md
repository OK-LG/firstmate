...
zcode is likewise verified for crewmate and scout launches ONLY (dated 2026-09-14 live pass on zcode-app-cli 3.11.2-24 wrapping zcode-runtime 0.16.5), refused for a secondmate for the same reason - no hook surface and no primary supervision protocol; [its adapter reference](../.agents/skills/harness-adapters/references/harness/zcode.md) owns the pass record and the `ZCODE_API_KEY` worker-credential precondition, because the launch template wires no credential itself.
