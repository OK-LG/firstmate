...
| Zcode | Uncovered | None. | Zcode registers no tracked session-open hook, so a Zcode primary takes the helm through the AGENTS.md section 3 instruction itself; a native digest delivery needs its own verified adapter and is deliberately deferred. |
