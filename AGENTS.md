...
state/               runtime records and signals; gitignored
  ...
  <id>.zcode-turnend-token  firstmate-owned zcode hook registry token for the task; removed by teardown
  <id>.zcode-session  zcode session id recorded by the turn-end hook; a same-harness relaunch reuses it through --resume; removed by teardown
...
The verified harnesses are `claude`, `codex`, `opencode`, `pi`, `pi-signed`, `grok`, `kimi`, `cursor`, `omp`, and `zcode`, plus `muse`, `gemini`, `rovo`, and `agy` for crewmates and scouts only; never dispatch on an unverified adapter.
