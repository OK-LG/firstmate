...
# isolated rendered-tail fallback; zcode joins that same pre-captured rendered
# tail feed (its classifier arm owns why it has no default signature); a herdr
# crew's native `busy` is accepted
...
crew_busy_verdict() {  # <target>
  local tail40=''
  case "$HARNESS" in
    grok*|zcode*) tail40=$(fm_backend_capture "$TASK_BACKEND" "$1" 40 "$EXPECTED_LABEL" 2>/dev/null) || tail40='' ;;
  esac
