...
case "$HARNESS" in
  claude|codex|opencode|pi|grok|cursor|omp|zcode) SNIPPET="$DOC_DIR/$HARNESS.md" ;;
...
repair_line() {
  ...
  case "$HARNESS" in
    ...
    grok)
      ...
      ;;
    zcode)
      printf '%s%s\n' "$prefix" 'repair missing watcher supervision with bin/fm-watch-arm.sh as its own zcode Bash tool background task (run_in_background), never shell &.'
      ;;
    cursor)
