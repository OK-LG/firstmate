...
crew_dispatch_validate() {
  ...
  err=$(jq -r '
    def verified($h): ["claude","codex","opencode","pi","pi-signed","grok","kimi","cursor","agy","muse","rovo","omp","zcode"] | index($h);
    def effort_ok($h; $m; $e):
      if $e == null then true
      ...
      elif $h == "opencode" or $h == "kimi" or $h == "cursor" or $h == "zcode" then false
      else true
      end;
    def profiles($value):
